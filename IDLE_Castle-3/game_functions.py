...
class Button:
    def __init__(
        self, x, y, width, height, text, inactive_color, active_color, action=None
    ):
    ...
    def draw(self, screen):
        ...
        if self.rect.collidepoint(mouse):
            pygame.draw.rect(
                screen, self.active_color, self.rect
            )  # Draw active color if mouse is over the button
            self.active = True
            ...
            pygame.draw.rect(
                screen, self.inactive_color, self.rect
            )  # Draw inactive color if mouse is not over the button
            self.active = False
...
class ImageButton(Button):
    def __init__(
        ...
        super().__init__(
            x, y, width, height, main_text, (0, 0, 0, 0), (0, 0, 0, 0), action
    ...
    def draw(self, screen):
        ...
        if self.rect.collidepoint(mouse):
            ...
            # Check for a click and execute the associated action
            if (
                click[0] == 1 and self.action is not None and not self.clicked
        ...
        sub_text_surface = self.sub_font.render(
            self.sub_text, True, self.sub_text_color
        )
        sub_text_rect = sub_text_surface.get_rect(
            midtop=(text_rect.centerx, text_rect.bottom + 5)
        )  # Adjust the position
...
def button_japan(
    ...
    button2 = ImageButton(
        ...
        button_image,
        "Main Keep",
        f"LVL:{player.mainKeepLVL} Price:{player.upgrading_price(player.mainKeepLVL + 1)}",
        font1,
        font2,
        text_color,
        text_color,
        action=lambda: player.upgrade_building("mainKeepLVL"),
    )
    button3 = ImageButton(
        950,
        180,
        button_image,
        "West Tower",
        f"LVL:{player.westTowerLVL} Price:{player.upgrading_price(player.westTowerLVL + 1)}",
        font1,
        font2,
        text_color,
        text_color,
        action=lambda: player.upgrade_building("westTowerLVL"),
    )
    button4 = ImageButton(
        950,
        250,
        button_image,
        "East Tower",
        f"LVL:{player.eastTowerLVL} Price:{player.upgrading_price(player.eastTowerLVL + 1)}",
        font1,
        font2,
        text_color,
        text_color,
        action=lambda: player.upgrade_building("eastTowerLVL"),
    )
    button5 = ImageButton(
        950,
        320,
        button_image,
        "Wall",
        f"LVL:{player.wallLVL} Price:{player.upgrading_price(player.wallLVL + 1)}",
        font1,
        font2,
        text_color,
        text_color,
        action=lambda: player.upgrade_building("wallLVL"),
    )
    button6 = ImageButton(
        950,
        390,
        button_image,
        "Dojo",
        f"LVL:{player.dojoLVL} Price:{player.upgrading_price(player.dojoLVL + 1)}",
        font1,
        font2,
        text_color,
        text_color,
        action=lambda: player.upgrade_building("dojoLVL"),
    )
    button7 = ImageButton(
        950,
        460,
        button_image,
        "Farm",
        f"LVL:{player.farmLVL} Price:{player.upgrading_price(player.farmLVL + 1)}",
        font1,
        font2,
        text_color,
        text_color,
        action=lambda: player.upgrade_building("farmLVL"),
    )
    button8 = ImageButton(
        950,
        530,
        button_image,
        "Weather",
        f"LVL:{player.weather} Price:{player.upgrading_price(player.weather + 1)}",
        font1,
        font2,
        text_color,
        text_color,
        action=lambda: player.upgrade_building("weather"),
    )
    button1.draw(screen)
    button2.draw(screen)
    button3.draw(screen)
...
def button_europa(button_image, font1, font2, text_color, screen, player):
    ...
    button6 = ImageButton(
        ...
        button_image,
        "Gate",
        f"LVL:{player.gateLVL} Price:{player.upgrading_price(player.gateLVL+1)}",
        font1,
        font2,
        text_color,
        text_color,
        action=lambda: player.upgrade_building("gateLVL"),
    )
    button7 = ImageButton(
        950,
        460,
...
def quit_button(screen):
    ...
    quit_button = Button(
        0, 0, 35, 35, "EXIT", (200, 100, 100), (200, 200, 200), action=pygame.quit
    )
